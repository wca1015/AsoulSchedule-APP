...
- **组合快捷筛选**：成员行下方提供组合胶囊——枝江 / A-SOUL / 小心思 / 嘉贝 / 乃贝 / 琳嘉，点一下 = 只看这几位的**同台多人场**（双人 / 团播，要求该场全部参与者都在组合内），不会把各自单播混进来；成员头像多选则是「任一参与」语义（含单播）。服务端下发的一期双人组合键（`bella_jiaran` / `bella_nailin` / `jiaran_nailin`）同样会命中两名参与成员。
- **内置成员库**（离线硬编码，无需网络）：
...
> 若服务端把团播按分组**粗粒度展开**（如「命题KTV」标为枝江综艺 → 展开 5 人），则按 ICS 更精确的成员列表
> 收窄（收窄后卡片改显示对应组合名，如「心宜思诺团播」）。
...
2. 上传**自动命名的产物**到 GitHub Release：`app/build/outputs/apk/publish/枝江直播日历.apk`（固定文件名，无需手动改名）
